...
jQuery.validator.addMethod('date', function(value, element) {
  ...
  $.each(value, function(i, o) {
    value[i] = parseInt(o, 10);
  });

  var format = element.attr('date-format')|| 'dd/mm/yyyy';
  var xdate;
  ...
  if (format === 'dd/mm/yyyy') {
    ...
    if (
      xdate.getFullYear() === value[2] &&
      xdate.getDate() === value[0] &&
      xdate.getMonth() + 1 === value[1] &&
      (xdate.getFullYear() + '').length === 4
    ) {
      return true;
    }
...
jQuery.validator.addMethod('mindate',function(v,el,minDate){	
    ...
    return xdate <= curDate;
}, 'Por favor ingresa una fecha válida');

// Validador de maxima fecha
...
jQuery.validator.setDefaults({
  errorPlacement: function(error, element) {
    var errorContent = $(element).parents('.ElementForm').find('.ElementForm-errorContent');
